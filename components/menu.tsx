...
export default function Menu(props: MenuProps) {
  ...
  return (
    <div className={showMenu 
      ...
      <button className='py-4 m-4 flex flex-col text-black  items-center justify-center hover:text-orange-500 active:text-amber-400' onClick={() => {setShowMenu(false); scroll.scrollToTop({ smooth: true, duration: 500 })}}>
          Home
        </button>
      {menuItems.map(i => (
        <Link onClick={() => {setShowMenu(false);  setTimeout(() => setShowHeader(false), 1000)}} activeClass="active" to={i.title.toLowerCase()} spy={true} smooth={true} offset={50} duration={500}  key={i.title}>
          <div className='hover:cursor-pointer py-4 m-4 flex flex-col text-black  items-center justify-center hover:text-orange-500 active:text-amber-400 '>
            {i.title}
          </div>
        </Link>
      ))}
